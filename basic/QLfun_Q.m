...
function like=QLfun_Q(xpar, beh_dat)
beta=xpar(1); 
alpha=xpar(2); 
gamma=xpar(3); 
S_=xpar(4);
...
%        reactions = beh_dat{i,1}(:,3);
%         reactions=exp(-reactions./lambda);
%         reactions=reactions/max(reactions);    
%             Rw_RT= rewards.*reactions;  
    nt = length(choices);
            v_right = 0.5;     v_left = 0.5;    
